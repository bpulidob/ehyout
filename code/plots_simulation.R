...
load_and_prepare_data <- function(outlier_rate,
  ...
  required_metrics_mean <- c("TPR_m", "FPR_m", "MCC_m", "BA_m", "Time_m", "F1_m")
  required_metrics_median <- c("TPR_med", "FPR_med", "MCC_med", "BA_med", "Time_med", "F1_med")
...
# Plot 1: Boxplot of Median F1
plot_main_box_f1 <- plot_summary_boxplots(
  data = main_paper_data,
  metric_col = "F1_med", #
  title = paste("Method Performance Comparison (Median F1, OR =", TARGET_OR_MAIN, ")"),
  ylab = "Median F1 per DGP"
)
print(plot_main_box_f1)

# Plot 1: Boxplot of Median MCC
plot_main_box_mccm <- plot_summary_boxplots(
  data = main_paper_data,
  metric_col = "MCC_m", # Use median MCC - often robust
  title = paste("Method Performance Comparison (Mean MCC, OR =", TARGET_OR_MAIN, ")"),
  ylab = "Mean MCC per DGP"
)
print(plot_main_box_mccm)
# ggsave(paste0("main_boxplot_mcc_med_or", TARGET_OR_MAIN*100, ".png"), plot_main_box_mcc, width = 8, height = 5)

# Plot 1: Boxplot of Mean BA
plot_main_box_bam <- plot_summary_boxplots(
  data = main_paper_data,
  metric_col = "BA_m", # Use median MCC - often robust
  title = paste("Method Performance Comparison (Mean MCC, OR =", TARGET_OR_MAIN, ")"),
  ylab = "Mean BA per DGP"
)
print(plot_main_box_bam)


# Plot 1: Boxplot of Mean F1
plot_main_box_f1m <- plot_summary_boxplots(
  data = main_paper_data,
  metric_col = "F1_m", #
  title = paste("Method Performance Comparison (Mean F1, OR =", TARGET_OR_MAIN, ")"),
  ylab = "Mean F1 per DGP"
)
print(plot_main_box_f1m)
